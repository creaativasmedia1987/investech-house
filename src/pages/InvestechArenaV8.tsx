...
import { Trophy, Zap, Plus, X, Building2, BarChart3, GitCompare, Settings, Trash2, Edit3, Lock, UserCircle2, Layers, PieChart as PieIcon, ListTodo, FileText, Printer, ArrowUpRight, ArrowDownRight, Swords, Flame, Target } from 'lucide-react';
import { BarChart, Bar, XAxis, YAxis, ResponsiveContainer, Cell, PieChart, Pie, LabelList } from 'recharts';
import confetti from 'canvas-confetti';

const InvestechArenaV8 = () => {
  // 1. ESTADO DE DADOS
  const [vendas, setVendas] = useState([
    ...
    { id: 5, corretor: 'Carla Souza', foto: 'https://i.pravatar.cc/150?u=carla', unidade: 'Apartamento', construtora: 'Celi', valor: 3100000, mes: 'Mar' }
  ]);
  ...
  const [slideAtivo, setSlideAtivo] = useState(0); // 0: Arena, 1: Corretores, 2: Tipos, 3: Construtoras, 4: Log de Vendas
  ...
  // 4. LÓGICA DA ARENA (DUELO)
  const dueloArena = useMemo(() => {
    const sorted = [...rankingCorretores];
    if (sorted.length < 2) return null;

    // Sorteia dois que estão próximos (ex: 1º vs 2º ou 2º vs 3º)
    const seed = Math.floor(Math.random() * (sorted.length - 1));
    return {
      p1: sorted[seed],
      p2: sorted[seed + 1],
      diff: sorted[seed].valor - sorted[seed + 1].valor
    };
  }, [rankingCorretores, slideAtivo]); // Recalcula ao trocar slide

  // 5. LÓGICA DO COMPARATIVO
    ...
    <div className="min-h-screen bg-[#050505] text-white p-6 font-sans flex flex-col relative overflow-hidden">
      ...
      <main className="flex-1 grid grid-cols-12 gap-8 mb-16 print:hidden z-10">
        ...
        <div className="col-span-8 bg-[#0a0a0a] rounded-[2.5rem] border border-white/5 p-10 relative overflow-hidden h-[500px]">
          ...
          {/* SLIDE 0: MODO ARENA (DUELO) */}
          <div className={`absolute inset-0 p-10 transition-all duration-1000 flex flex-col items-center justify-center ${slideAtivo === 0 ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
            <div className="absolute top-10 text-[#c5a059] font-black tracking-[0.5em] uppercase text-xs flex items-center gap-2">
              <Flame size={16} /> Desafio {mesSelecionado} <Flame size={16} />
            </div>

            {dueloArena ? (
              <div className="flex items-center gap-20 w-full justify-center mt-8">
                {/* LUTADOR 1 */}
                <div className="text-center group">
                  <div className="relative mb-6">
                    <div className="absolute -inset-4 bg-[#c5a059]/20 blur-2xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity"></div>
                    <img src={dueloArena.p1.foto || `https://i.pravatar.cc/300?u=${dueloArena.p1.nome}`} className="w-56 h-56 rounded-full border-4 border-[#c5a059] object-cover relative z-10 shadow-[0_0_50px_rgba(197,160,89,0.3)]" alt="P1" />
                    <div className="absolute bottom-0 right-0 bg-[#c5a059] text-black px-4 py-1 font-black italic text-xl skew-x-[-12deg] z-20">R$ {(dueloArena.p1.valor/1000).toFixed(0)}k</div>
                  </div>
                  <h2 className="text-3xl font-black italic uppercase tracking-tighter">{dueloArena.p1.nome}</h2>
                </div>

                {/* VS ICON */}
                <div className="relative">
                  <div className="text-8xl font-black italic text-white/10 absolute -inset-10 flex items-center justify-center pointer-events-none">VS</div>
                  <div className="bg-white text-black w-24 h-24 rounded-full flex items-center justify-center font-black text-4xl italic relative z-10 border-8 border-[#0a0a0a] shadow-[0_0_30px_rgba(255,255,255,0.2)]">VS</div>
                </div>

                {/* LUTADOR 2 */}
                <div className="text-center group">
                  <div className="relative mb-6">
                    <div className="absolute -inset-4 bg-red-500/10 blur-2xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity"></div>
                    <img src={dueloArena.p2.foto || `https://i.pravatar.cc/300?u=${dueloArena.p2.nome}`} className="w-56 h-56 rounded-full border-4 border-gray-800 object-cover relative z-10" alt="P2" />
                    <div className="absolute bottom-0 left-0 bg-white text-black px-4 py-1 font-black italic text-xl skew-x-[12deg] z-20">R$ {(dueloArena.p2.valor/1000).toFixed(0)}k</div>
                  </div>
                  <h2 className="text-3xl font-black italic uppercase tracking-tighter text-gray-400">{dueloArena.p2.nome}</h2>
                </div>
              </div>
            ) : (
                <div className="text-center">
                   <Target size={80} className="text-[#c5a059] mx-auto mb-6 opacity-20" />
                   <h2 className="text-4xl font-black italic text-white/20 uppercase tracking-tighter">Buscando Desafiantes...</h2>
                 </div>
            )}

            {dueloArena && (
              <div className="absolute bottom-10 bg-white/5 px-10 py-4 rounded-full border border-white/10 backdrop-blur-md">
                <p className="text-sm font-bold tracking-widest uppercase italic text-gray-400">
                  Diferença de apenas <span className="text-[#c5a059]">R$ {(dueloArena.diff/1000).toFixed(0)}k</span>. Quem fecha a próxima?
                </p>
              </div>
            )}
          </div>
          ...
          {/* SLIDE 4: LOG DE VENDAS */}
          <div className={`absolute inset-0 p-10 transition-all duration-1000 ${slideAtivo === 4 ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
      ...
      {/* RODAPÉ STATUS (ARENA / TICKER) */}
      <footer className="fixed bottom-0 left-0 w-full bg-[#c5a059] py-3 overflow-hidden border-t-4 border-black/20 z-10 print:hidden flex items-center justify-between">
        <div className="whitespace-nowrap animate-marquee inline-block text-black font-black text-xs uppercase italic flex-1">
        ...
        <div className="bg-black/20 px-4 py-1 flex items-center gap-2 mr-4 rounded-full text-[10px] text-black font-black uppercase">
          <div className="w-2 h-2 rounded-full bg-green-900 animate-pulse"></div> Local Mode
        </div>
      </footer>
              ...
              <div className="flex justify-between items-start mb-10 border-b border-white/5 pb-8 print:border-gray-200">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-[#c5a059] font-bold tracking-[0.3em] uppercase text-xs print:text-gray-500">{mesA} vs {mesB}</p>
                    </div>
                </div>
      ...
      {/* BACKGROUND EFFECTS */}
      <div className="fixed top-[-10%] left-[-10%] w-[40%] h-[40%] bg-[#c5a059]/5 blur-[120px] rounded-full pointer-events-none z-0"></div>
...
export default InvestechArenaV8;
